...
[title-slide] Style for the opening/title slide of your presentation. See <tt>example/one/slidesA.md</tt>.
[center] centers images on a slide
[full-page] allows an image to take up the whole slide
[bullets] sizes and separates bullets properly (fits up to 5, generally). See <tt>example/one/slidesA.md</tt>.
[smbullets] sizes and separates more bullets (smaller, closer together)
[subsection] creates a different background for section divider slides. See <tt>example/two/slidesA.md</tt>.
[command] monospaces h1 title slides
[commandline] for pasted commandline sections (needs leading '$' or '#' for commands, then output on subsequent lines). See <tt>example/one/slidesB.md</tt>.
[code] monospaces everything on the slide
[incremental] can be used with 'bullets' and 'commandline' styles, will incrementally update elements on arrow key rather than switch slides. See <tt>example/one/slidesA.md</tt>.
[inverse] changes to a dark background and light text.
[small] make all slide text 80%
[smaller] make all slide text 70%
[execute] on Javascript, Coffeescript, Ruby, and Shell highlighted code slides, you can click on the code to execute it and display the results on the slide. See <tt>example/two/slidesA.md</tt>.
...
[toc] this slide will not appear in the table of contents
[cover] exclude this slide from the table of contents (alias for toc)
...
[bigtext] This will enlarge all the contents of the slide to the maximum possible size. <em>Caution, this should only be used for slides with just a few words or short phrases!</em> See <tt>example/bigtext/slides.md</tt>.
[autoplay] If you've included videos on the slide, this style will cause them to play when you switch to this slide. See <tt>example/embedding/slides.md</tt>.
...
Check out the <tt>example/</tt> directory included to see examples of most of these.
...
[.callout] Basic callout with an outlined box (no icon)
[.callout.info] Information callout with an info icon
[.callout.warning] Warning callout with an exclamation triangle icon
[.callout.question] Question callout with a question mark icon
[.callout.exercise] Exercise callout with a pencil icon
[.callout.stop] Stop/danger callout with a ban icon
[.callout.thumbsup] Positive callout with a thumbs up icon
[.callout.conversation] Discussion callout with a speech bubble icon
[.callout.glossary] Glossary term callout with a book icon
[.callout.terminal] Terminal/command callout with a terminal icon
...
Example usage:
    ...
    .callout.stop This indicates something critical.

See <tt>example/one/callouts.md</tt> for a complete example of all callout styles.
...
Slide transitions are powered by {Swiper.js}[https://swiperjs.com/] (version 11).
Transitions can be supplied through the use of <tt>transition=tname</tt> on the
<tt>!SLIDE</tt> definition:
...
- none (instant transition, the default)

The fade transition provides a smooth crossfade effect between slides.

See <tt>example/one/slidesA.md</tt> for examples of transition usage.
...
Standard Markdown image syntax is supported. Image paths are relative to the
slide's current directory:

    ![Alt text](image.png)

You can also style images by including a class name as the first word:

    ![.centered My Image](path/to/image.png)

== Videos

HTML5 video can be embedded directly in slides:
...
Note that video paths should use the <tt>file/</tt> prefix and be relative to
the presentation root directory.

Use the <tt>autoplay</tt> slide style to automatically start videos when the
slide is displayed:
...
See <tt>example/embedding/slides.md</tt> for complete examples of image and video embedding.

= Language Highlighting

Showoff uses {Highlight.js}[https://highlightjs.org] (version 11.11.1) to highlight
...
add the <tt>numbers</tt> keyword, it will be rendered with line numbers.
...
See <tt>example/two/slidesA.md</tt> for code highlighting examples including
...
Highlighting styles include (104 themes available):

    agate                 dracula               monokai-sublime
    androidstudio         far                   obsidian
    arduino-light         foundation            paraiso-dark
    arta                  github                paraiso-light
    ascetic               github-gist           pojoaque
    atelier-cave-dark     googlecode            qtcreator_dark
    atelier-cave-light    grayscale             qtcreator_light
    atelier-dune-dark     gruvbox-dark          railscasts
    atelier-dune-light    gruvbox-light         rainbow
    (and more...)         hopscotch             school_book
    brown_paper           hybrid                solarized_dark
    codepen-embed         idea                  solarized_light
    color-brewer          ir_black              sunburst
    dark                  kimbie.dark           tomorrow
    darkula               kimbie.light          tomorrow-night
    default               magula                tomorrow-night-blue
    docco                 mono-blue             tomorrow-night-bright
                          monokai               tomorrow-night-eighties
                          monokai_sublime       vs
                                                xcode
                                                zenburn

See the <tt>public/css/highlight/</tt> directory for all available themes.
...
<tt>nochrome</tt> if you don't want the window titlebars drawn, or <tt>code_wrap</tt> if you
...
<br>
    ...
    @@@ PowerShellConsole nochrome code_wrap
    PS C:\> echo "This is a PowerShell window with no titlebar and wrapped text"

See <tt>example/highlights/02console.md</tt> for more console examples.


== Rendering Diagrams

Showoff includes the {Mermaid}[https://mermaid.js.org/] diagram renderer (version 11.12.2),
which allows you to describe diagrams with a simple language and style them with CSS.
To do so, create a code block using a highlight style of <tt>render-diagram</tt>.
...
Example Gantt chart:
    ...
    gantt
        ...
        section Phase 1
        Task 1           :done,    t1, 2024-01-01, 2024-01-07
        Task 2           :active,  t2, 2024-01-08, 7d
        section Phase 2
        Task 3           :         t3, after t2, 5d

See https://mermaid.js.org/intro/ for the full syntax documentation and examples.
...
See <tt>example/three/js-events.md</tt> for complete working examples of custom events.
...
See <tt>example/four/01slide.md</tt> and <tt>example/simple.tpl</tt> for a
working template example. The <tt>example/showoff.json</tt> shows how templates
are configured.
