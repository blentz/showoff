...
RSpec.describe CommandlineParser do
  ...
  it 'parses a single command with output' do
    ...
    expect(cmd[:prompt]).to eq('$')
    expect(cmd[:input]).to eq('echo hi')
    expect(cmd[:output]).to include('hello world')
  end
  ...
  it 'parses multiple commands with different prompts' do
    ...
    expect(tree[0][:command][:prompt]).to eq('#')
    expect(tree[1][:command][:prompt]).to eq('>>')
  end
  ...
  it 'parses multiline input lines ending with backslashes' do
    ...
    expect(cmd[:output]).to include('ok')
  end
  ...
  it 'handles optional output blocks and blank lines' do
    ...
    expect(cmd[:output]).to include('Tue Dec')
  end
end
